...
// Ça génère toutes les pages /mandats/1, /mandats/2… à la build.
export async function generateStaticParams() {
    const mandats = await prisma.mandat.findMany({
        select: {id: true}
    })
    return mandats.map(mandat => ({id: mandat.id.toString()}))
}
        ...
        <div className="container mx-auto p-4">
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="w-full lg:w-2/3 flex flex-col gap-8">
                    ...
                    {/* Bloc infos en mobile */}
                    <div className="block lg:hidden bg-gray-100 rounded-xl space-y-4 pl-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 justify-items-center">
                        {similaires.map((annonce) => (
                            <AnnonceCardHeader key={annonce.id} annonce={annonce} />
                        ))}
                    </div>
                ...
                {/* Colonne droite */}
                <div className="w-full lg:w-1/3 flex flex-col gap-8 rounded-2xl">
                    {/* Bloc infos desktop */}
                    <div className="hidden lg:block bg-gray-100 rounded-xl space-y-4 pl-4">
                        <h2 className="text-3xl font-bold">
                            {mandat.prix.toLocaleString()} €
                        </h2>
                        <h2 className="text-gray-600 text-2xl">{mandat.cp} {mandat.ville}</h2>
